...
public class DashboardService {
    ...
    public DashboardService(FinancialProductRepository productRepository,
                            EvaluationProjectRepository projectRepository,
    ...
    public DashboardResponse getDashboard() {
        ...
        List<ProjectEvaluationResponse> recent = projectRepository.findAll().stream()
                .limit(3)
                .map(p -> projectEvaluationService.evaluate(p.getId()))
                .toList();
        List<ProjectComparisonPoint> comparisons = snapshotRepository.findAll().stream()
                .collect(java.util.stream.Collectors.toMap(
                        snapshot -> snapshot.getProject().getId(),
                        snapshot -> snapshot,
                        (left, right) -> left.getEvaluatedAt().isAfter(right.getEvaluatedAt()) ? left : right
                ))
                .values().stream()
                .sorted(java.util.Comparator.comparing(ProjectEvaluationSnapshot::getNpv).reversed())
                .map(snapshot -> new ProjectComparisonPoint(
                        snapshot.getProject().getId(),
                        snapshot.getProject().getName(),
                        round(snapshot.getNpv()),
                        round(snapshot.getIrr()),
                        round(snapshot.getRiskScore()),
                        snapshot.getGrade()
                ))
                .limit(5)
                .toList();
